...
do
	for j in `seq 0 20`;
	do
		qsub -q atlas -v num_particles=25,include_ignored_gt=False,include_dontcare_in_gt=False,use_regionlets_and_lsvm=True,sort_dets_on_intervals=True,RUN_IDX=$i,NUM_RUNS=$NUM_RUNS,SEQ_IDX=$j setup_rbpf_python_venv.sh
	done
done
